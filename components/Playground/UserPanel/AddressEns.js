...
const AddressEns = ({ address }) => {
  ...
  if (ensLoading || !address) {
    return <div className="flex w-full justify-center h-10 bg-slate-200 dark:bg-slate-800 rounded-xl animate-pulse"></div>
  }

  return (
    <div className="flex flex-row space-x-1 h-10 items-center justify-center w-full">
      <div className={(ensData && ensData.length > 18 ? "text-md" : "text-xl")}>{ensData ? ensData : address.substring(0, 6) + "..." + address.substring(address.length - 4)}</div>
