...
const DaoUtilityBar = () => {
  return (
    <div className="w-full h-0 flex flex-row space-x-1 items-end justify-end">
      ...
      <button className="flex -transform-y-6 h-6 w-6 items-center justify-center rounded-full border text-white border-slate-400 bg-slate-200 p-1 shadow hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-600">
        <HiStar />
      </button>
    </div>
  )
